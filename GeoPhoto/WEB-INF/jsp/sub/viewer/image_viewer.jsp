...
String map = request.getParameter("map"); // 맵 종류(OSM, VW)
String mapMode = request.getParameter("mapMode"); // VW일 경우, mapMode
%>
<style type="text/css">
...
function getOneImageData(){
	...
	var param		= "one/" + loginToken + "/" + loginId + "/&nbsp/&nbsp/&nbsp/"+idx+"/&nbsp";
...
function addImageMoveList(){
	...
	var param		= loginToken + "/" + loginId + "/list/" + projectIdx + "/" + pageNum + '/' + contentNum + '/nbsp';
	var callBack	= "?callback=?";
...
function imageControllView(type){
	...
	if(type == 'Y'){
		...
		if(imgEditMode == 2){
			$('#openlayers_gray').get(0).contentWindow.grayMarkerSet('cancel');
...
// 		tmpDiv += "</div>";
// 		$('body').append(tmpDiv);
		$('#openlayers_gray').get(0).contentWindow.graySetCenter();
		var dMarkerLat = 0;		//default marker latitude
		var dMarkerLng = 0;		//default marker longitude
		var dMapZoom = 10;		//defalut map zoom
...
function newMarkerSave(){
	$('#openlayers_gray').get(0).contentWindow.grayMarkerSet('ok');
...
function reloadMap(type) {
	...
	$('#openlayers').get(0).contentWindow.setCenter(arr[0], arr[1], 1);
}
...
<!-- <div id='exif_dialog' style='position:absolute; left:800px; top:310px; width:300px; height:200px; border:1px solid #999999; display:block; font-size:13px;'> -->
	<div class='mapLoading' style="position:absolute; left:781px; top:0px; width:352px; display:block; font-size:13px; height: 30px; border-bottom : 1px solid #ebebeb;">
	...
	<div class='mapLoading' id='image_exif_area' style='width: 352px;background: #ffffff;position: absolute;top: 31px;border-bottom: 1px solid rgb(235, 235, 235);left: 781px;z-index:2; display:none;'>
		<!-- EXIF 삽입 다이얼로그 객체 -->
		...
		<div id="tabsChild_1" style='height: 195px;' class="selectExifTabChild" style="display:none;">
			<table id='normal_exif_table' style="margin-top: 10px;">
				...
				<tr><td colspan="4">
					<button class="smallWhiteBtn" style="width:120px; height:25px; display:block;float: right;" onclick="imageWrite();" id="makeImageBtn">Edit Annotation</button>
				</td></tr>		
			</table>
		</div>
		<div id="tabsChild_2" style='height: 235px; overflow-y:scroll;' class="noSelectExifTabChild" style="display:none;">
...
<!-- 지도 영역 -->
<div id='image_map_area' class='mapLoading' style='position: absolute;left: 781px;top: 31px;width: 352px;height: 514px;display: block;z-index: 1;'>
...
<div id='image_map_area_gray' style='position:absolute; left:0px; top:0px; width:1135px; height:550px; display:none; background-color:#999;'>
	<iframe id='openlayers_gray' src='<c:url value="/geoPhoto/image_openlayers.do"/>' style='width:100%; height:100%; margin:1px; border:none;'></iframe>
</div>
