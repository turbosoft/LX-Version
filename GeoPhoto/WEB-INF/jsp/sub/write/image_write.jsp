...
// 	$('#save_dialog').dialog('close');
	...
	var fullHeight = 944;
	var barWidth = 60;
	var imgWidth = $('img#image_write_canvas').width();
	var imgHeight = $('img#image_write_canvas').height();
	var imgOffset = $('img#image_write_canvas').offset();
	var imgTop = imgOffset.top;
	var imgLeft = imgOffset.left - barWidth;
//	alert("Image Width : " + imgWidth);
//	alert("Image Height : " + imgHeight);
//	alert("Image offset top : " + imgTop);
//	alert("Image offset left : " + imgLeft);
		...
		if(id!='image_write_canvas') {
			if(id.indexOf("c")!=-1) {
				...
				for(var j=0; j<geometry_total_arr_buf_1.length; j++) {
					...
					// kangih99 20200113
					// 좌표변환
					top = Math.round(parseInt(top) * fullHeight / imgHeight) - imgTop;
					left = Math.round(parseInt(left) * fullWidth / imgWidth) - imgLeft;

					var xarr = x_str.split("_");
					var yarr = y_str.split("_");
					for (var idx=0; idx<xarr.length; idx++){
//						xarr[idx] = Math.round(parseInt(xarr[idx]) * fullWidth / imgWidth);
						xarr[idx] = Math.round(parseInt(xarr[idx]) * fullWidth / imgWidth) - imgLeft;
					}
					for (var idx=0; idx<yarr.length; idx++){
//						yarr[idx] = Math.round(parseInt(yarr[idx]) * fullHeight / imgHeight);
						yarr[idx] = Math.round(parseInt(yarr[idx]) * fullHeight / imgHeight) - imgTop;
					}
...
//						x = Math.round(parseInt(buf1[i]) * imgWidth / fullWidth);
						x = Math.round(parseInt(buf1[i]) * imgWidth / fullWidth) + imgLeft;
//						x = Math.round((parseInt(buf1[i]) + imgLeft) * imgWidth / fullWidth);
						geometry_point_arr_1.push(x);
					}
					var buf2 = y_str.split('_');
					for(var i=0; i<buf2.length; i++) {
//						y = Math.round(parseInt(buf2[i]) * imgHeight / fullHeight);
						y = Math.round(parseInt(buf2[i]) * imgHeight / fullHeight) + imgTop;
//						y = Math.round((parseInt(buf2[i]) + imgTop) * imgHeight / fullHeight);
						geometry_point_arr_2.push(y);
					}
					createGeometry(type);
				}
...
function reloadMap(type) {
	var arr = readMapData();
	$('#openlayers').get(0).contentWindow.setCenter(arr[0], arr[1], 2);
	if(type==2) {
		if(imgDroneType != null && imgDroneType == 'Y'){
			$('#openlayers').get(0).contentWindow.drawCircleOnMap(arr[0], arr[1], 100, 1);
		}else{
			$('#openlayers').get(0).contentWindow.setAngle(arr[2], arr[3]);
		}
	}
}
...
function setExifData(lat_str, lng_str) {
...
function detectImage(){
	//url: 'http://djr.urbanai.net/task?url=http://106.246.9.37:8127/GeoCMS/upload/GeoPhoto/'+file_url+'&model_name=mscoco_maskrcnn&file_type=image',
	var fileDivide = file_url.split(".");
	var Url			= baseRoot() + "pgm/urlOption/"+fileDivide[0]+"/"+fileDivide[1];
	var callBack	= "?callback=?";
...
function detectImage2()
{
	...
	$.ajax({
		//url: 'http://deepapi.u-gis.net/?command=detect&model=mscoco&img=http://localhost:8127/GeoCMS/upload/GeoPhoto/'+file_url,
		url: 'http://202.31.147.180/task?url=http://turbo.hopto.org:8127/GeoCMS/upload/GeoPhoto/'+file_url+'&model_name=mscoco_maskrcnn&file_type=image',
		dataType: 'json',
		beforeSend:function(){
			$.loadingBlockShow();
		},
...
function fnImageImportVgg(){
	...
	$('#file').change(function(){
		if($(this).val()){
			...
			$.ajax({
				type	: "POST"
				, url	: "/GeoCMS/geoImageImportVgg.do?idx=<%= idx %>&file_url=<%= file_url %>"
				, dataType	: "xml"
				, data	: formData
				, async	: false
				, cache	: false
...
function fnImageImport(){
	...
	$('#file').change(function(){
		if($(this).val()){
			...
			$.ajax({
				type	: "POST"
				, url	: "/GeoCMS/geoImageImport.do?idx=<%= idx %>&file_url=<%= file_url %>"
				, dataType	: "xml"
				, data	: formData
				, async	: false
				, cache	: false
...
function fnImageImportDetect(){

	var formData = new FormData($('#formImport')[0]);
	$.ajax({
		type	: "POST"
		, url	: "/GeoCMS/geoImageImportDetect.do?idx=<%= idx %>&file_url=<%= file_url %>"
		, dataType	: "json"
		, data	: formData
		, async	: false
		, cache	: false
		...
		, success: function(data) {
			...
			if (data.Code == 100){
				alert('검사 작업을 완료하였습니다. 페이지가 다시 시작됩니다.');
				location.reload();
			} else {
				alert('검사 작업에 오류가 있습니다 : ' + data.Message);
			}
		}
		, error:function(request,status,error){
			alert("code:"+request.status+"\n"+"message:"+request.responseText+"\n"+"error:"+error);
...
function fnImageExport(){
	...
	$('<a href="/GeoCMS/geoImageExport.do?file_name=' + xml_file_name + '" target="_blank"></a>')[0].click();
}
...
function fnImageExportVgg(){
	...
	$('<a href="/GeoCMS/geoImageExportVgg.do?file_name=' + xml_file_name + '&kind=vgg" target="_blank"></a>')[0].click();
}
...
<!-- <div id='exif_dialog' style='position:absolute; left:800px; top:310px; width:300px; height:200px; border:1px solid #999999; display:block; font-size:13px;'> -->
	...
	<div id='image_exif_area' style='width: 292px; background: #ffffff;position: absolute;top: 31px;border-bottom: 1px solid rgb(235, 235, 235);left: 841px;z-index:2;display:none;'>
		<!-- EXIF 삽입 다이얼로그 객체 -->
	...
	<!-- <div id="detectBtn" class="smallGreyBtn" onclick="fnImageImportDetect();" style="width: 60px;height: 20px;float: right;border-radius:5px;text-align: center;position: absolute;top: 511px;left: 740px;cursor: pointer;font-size: 13px;">Detect</div> -->
	<div id="detectBtn" class="smallGreyBtn" onclick="fnImageImportDetect();" style="width: 60px;height: 20px;float: right;border-radius:5px;text-align: center;position: absolute;top: 511px;left: 740px;cursor: pointer;font-size: 13px;">Detect</div>
...
<div id='image_map_area' style='position: absolute;left: 841px;top: 31px;width: 292px;height: 514px;display: block;z-index: 1;'>
	<iframe id='openlayers' src='<c:url value="/geoPhoto/image_openlayers.do"/>' style='width:100%; height:100%; margin:1px; border:none;'></iframe>
</div>
<!-- EXIF 영역 -->
...
<div style="width:100%; position: absolute;top:550px;" align="center" class="writer_btn_class">
	...
	<button class='smallBlueBtn' style='margin-top:15px;margin-left:5px;' onclick='fnImageImport();'>Import</button>
	<button class='smallBlueBtn' style='margin-top:15px;' onclick='fnImageExport();'>Export</button>

	<button class='smallBlueBtn' style='margin-top:15px;margin-left:5px;' onclick='fnImageImportVgg();'>Import(VGG)</button>
	<button class='smallBlueBtn' style='margin-top:15px;' onclick='fnImageExportVgg();'>Export(VGG)</button>

	<button class='smallBlueBtn' style='margin-top:15px;margin-left:5px;' onclick='saveImageWrite1(2);'>Save</button>
