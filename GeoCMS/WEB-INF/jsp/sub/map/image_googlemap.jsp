...
<script type="text/javascript"
	src="http://map.vworld.kr/js/vworldMapInit.js.do?&apiKey=FB106A62-A791-3EE5-8B7B-5DBC8B408273"></script>
<script type="text/javascript"
	src="http://maps.googleapis.com/maps/api/js?key=AIzaSyAth-_FyQxRomNh2JkI_MvAWXRJuLOEXNI&v=3.exp&sensor=false&libraries=places,geometry&language=en&region=ER"></script>
<!-- <script type="text/javascript" src="http://maps.googleapis.com/maps/api/js?key=AIzaSyC9OgAFV4gO62GKAGO_uhVSwgp8aEIvAKQ"></script> -->
<script type='text/javascript'>
...
function mapCenterChange(objArr, selectId, i, gpsArrLat, gpsArrLon, objXml){		//tempObj: lat, lon, file, idx, dataKind, origin, thumbnail, id
	...
	var xml = tempArr[9];
	var innerHTMLStrLog = "";
	var innerHTMLStrFile = "";
	var innerHTMLStrAnno = "";
	/* $('.editAnno').each(function(idx,val){
		$(val).removeClass('editAnno');
		$(val).css('border','2px solid #888888');
		$(''+val+'#subDetailView_0').empty();
		//$(val).parent().attr('#subDetailView_0').empty();
		//$(''+selectId+'#subDetailView_0').empty();
	}); */
	//var idVal = $('#'+selectId).parent().attr('id').split("_");
	if($("#checkSub_"+i).is(":checked"))
	{
		var fileName = file.length>25? file.substring(0,25)+'...' : file;
		$('#Pro_'+ tempArr[4] +'_'+ tempArr[3]).addClass('editAnno');
		//$('#Pro_'+ tempArr[4] +'_'+ tempArr[3]+ ' #subDetailView').append("<li> lat = "+lat+" lon = "+lon+"</li>");
		innerHTMLStrLog += '<li style="border-bottom: 0.5px solid #F2F3F5; border: 1px solid black; background: white; font-weight: 600; margin: 0px 0px 0px 72px;"><div style="padding:7px;">';
		innerHTMLStrLog += '<input type="checkbox" id="checkGps_'+i+'" style="float:left;" class="cb1" onclick="lookGps(\''+gpsArrLat+'\',\''+gpsArrLon+'\',\''+i+'\');"/>';
		innerHTMLStrLog += '<label for="checkGps_'+i+'"></label>';
		innerHTMLStrLog += '<label class="titleLabel" style="width:130px !important;font-size: 15px;margin-top:3px;">GPS</label></div></li>';
		innerHTMLStrLog += '<div id="subGpsView_'+i+'" style="font-size: 15px;"></div>';
		$('#'+selectId).parent().parent().find('#subDetailView_'+i).append(innerHTMLStrLog);
		
		
		innerHTMLStrFile += '<li style="border-bottom: 0.5px solid #F2F3F5; border: 1px solid black; background: white; font-weight: 600; margin: 0px 0px 0px 72px;"><div style="padding:7px;">';
		innerHTMLStrFile += '<input type="checkbox" id="checkFile_'+i+'" style="float:left;" class="cb1" onclick="lookFile(\''+file+'\',\''+i+'\');"/>';
		innerHTMLStrFile += '<label for="checkFile_'+i+'"></label>';
		innerHTMLStrFile += '<label class="titleLabel" style="width:130px !important;font-size: 15px;margin-top:3px;">FILE</label></div></li>';
		innerHTMLStrFile += '<div id="subFileView_'+i+'" style="font-size: 15px;"></div>';
		
		$('#'+selectId).parent().parent().find('#subDetailView_'+i).append(innerHTMLStrFile);
		//$('#'+selectId).parent().parent().find('#subDetailView_'+i).append("<li style='border-bottom: 0.5px solid #F2F3F5; border: 1px solid black; background: white; font-weight: 600; margin: 0px 0px 0px 72px;'><div style='padding:7px;'> <label title="+file+">fileName = "+fileName+"</label></div></li>");
		
		innerHTMLStrAnno += '<li style="border-bottom: 0.5px solid #F2F3F5; border: 1px solid black; background: white; font-weight: 600; margin: 0px 0px 0px 72px;"><div style="padding:7px;">';
		innerHTMLStrAnno += '<input type="checkbox" id="checkAnno_'+i+'" style="float:left;" class="cb1" onclick="lookAnno(\''+objXml+'\',\''+i+'\');"/>';
		innerHTMLStrAnno += '<label for="checkAnno_'+i+'"></label>';
		innerHTMLStrAnno += '<label class="titleLabel" style="width:130px !important;font-size: 15px;margin-top:3px;">ANNOTATION</label></div></li>';
		innerHTMLStrAnno += '<div id="subAnnoView_'+i+'" style="font-size: 15px;"></div>';
		
		$('#'+selectId).parent().parent().find('#subDetailView_'+i).append(innerHTMLStrAnno);
		$('#Pro_'+ tempArr[4] +'_'+ tempArr[3]+ ' #subDetailView_'+i).append("<li> fileName = "+file+"</li>");
		$('#Pro_'+ tempArr[4] +'_'+ tempArr[3]+ ' img').css('border','2px solid red');
	}
	else
	{
		$("#subDetailView_"+i).empty();
		$('#Pro_'+ tempArr[4] +'_'+ tempArr[3]).removeClass('editAnno');
	}
	/* if($('#'+selectId+' img').hasClass('editAnno'))
	{
		$('#'+selectId).parent().find('#subDetailView').empty();
		$('#'+selectId+' img').removeClass('editAnno');
		$('#'+selectId+' img').css('border','2px solid #888888');
	}
	else
	{
		var fileName = file.length>25? file.substring(0,25)+'...' : file;
		...
		//$('#Pro_'+ tempArr[4] +'_'+ tempArr[3]+ ' #subDetailView').append("<li> lat = "+lat+" lon = "+lon+"</li>");
		$('#'+selectId).parent().parent().find('#subDetailView_'+idVal[1]).append("<li style='border-bottom: 0.5px solid #F2F3F5; border: 1px solid black; background: white; font-weight: 600; margin: 0px 0px 0px 72px;'><div style='padding:7px; font-size:15px;'> lat = "+lat+" lon = "+lon+"</div></li>");
		$('#'+selectId).parent().parent().find('#subDetailView_'+idVal[1]).append("<li style='border-bottom: 0.5px solid #F2F3F5; border: 1px solid black; background: white; font-weight: 600; margin: 0px 0px 0px 72px;'><div style='padding:7px; font-size:15px;'> <label title="+file+">fileName = "+fileName+"</label></div></li>");
		$('#Pro_'+ tempArr[4] +'_'+ tempArr[3]+ ' #subDetailView').append("<li> fileName = "+file+"</li>");
		$('#Pro_'+ tempArr[4] +'_'+ tempArr[3]+ ' img').css('border','2px solid red');
	} */
...
function lookGps(gpsArrLat, gpsArrLon, num)
{
	if($("#checkGps_"+num).is(":checked") == true)
	{
		...
		for(var i=0; i<gpsArrLatgo.length; i++)
		{
			...
			innerHTMLGps += '<label for="checkSubGps" style="background:#bebebe;"></label>'
			...
			$("#subGpsView_"+num).append("<li style='border-bottom: 0.5px solid #F2F3F5; border: 1px solid black; background: white; font-weight: 600; margin: 0px 0px 0px 108px;'><div style='padding:7px;'>"+innerHTMLGps+"</div></li>");
		}
	}
	else
	{
...
function lookFile(file, num)
{
	...
	if($("#checkFile_"+num).is(":checked") == true)
	{
		...
		$("#subFileView_"+num).append("<li style='border-bottom: 0.5px solid #F2F3F5; border: 1px solid black; background: white; font-weight: 600; margin: 0px 0px 0px 108px;'><div style='padding:7px;'>"+innerHTMLFile+"</div></li>");	
	}
	else
	{
...
function lookAnno(xml, num)
{
	...
	if($("#checkAnno_"+num).is(":checked") == true)
	{
		...
		for(var i=0; i<xmlObj.length; i++)
		{
			...
			$("#subAnnoView_"+num).append("<li style='border-bottom: 0.5px solid #F2F3F5; border: 1px solid black; background: white; font-weight: 600; margin: 0px 0px 0px 108px;'><div style='padding:7px;'>"+innerHTMLAnno+"</div></li>");
		}		
	}
	else
	{
...
{
	popup.openPop(selectNum,selectVal);
	popup.focus();
}
...
<body style='margin: 0px; padding: 0px;'>
	<div class="viewModeCls" style="display: none;">
		<input type="checkbox" id="polygonView"
			onclick="mapPolygonView(this);" style="vertical-align: middle;" /> <label
			style="margin-top: 3px; display: inline-block;">View Mode</label>
		...
		<div id="copyPojectAddDig">
			<table style="width: 100%;">
				<tr>
					...
					<td><input type="text" id="copyProjectNameTxt"
						style="width: 100%;" /></td>
				</tr>
				<tr style="text-align: center;">
					<td colspan="2"><input type="button" id="saveBtn" value="Save"
						onclick="addCopyPoject();" /> <input type="button" value="Cancel"
						onclick="closeCopyPoject();" /></td>
				</tr>
			</table>
		</div>
	</div>
	...
	<div onclick="makeSequenceOpen();return false;" id="copyReqStart"
		class="copyCssClass" style="display: none;">Selecting the area
		to copy</div>
	<div onclick="makeSequenceColse();return false;" id="copyReqExit"
		class="copyCssClass" style="display: none;">Cancel copy</div>
	<div onclick="makeSequenceData('In');return false;" id="copyReqNewIn"
		class="copyReqClass copyCssClass" style="display: none;">Create
		as selection</div>
	<div onclick="makeSequenceData('Out');return false;" id="copyReqNewOut"
		class="copyReqClass copyCssClass" style="display: none;">Create
		out of selection</div>
	<div onclick="makeSequenceBounding('On');return false;"
		id="copyReqNewOK" class="copyReqClass copyCssClass"
		style="display: none;">SELECT OK</div>

	<!-- center setting -->
	<div class="morkerModeCls1" id="morkerModeOpen"
		onclick="defaultMarkerSet('open');"
		style="display: none; height: 30px; width: 130px; position: absolute; left: 100px; top: 10px; z-index: 9; background-color: #ffffff; text-align: center; cursor: pointer;">
		<div style="margin-top: 3px; display: inline-block;">Set default
			location</div>
	</div>
	<div class="morkerModeCls2" id="morkerModeCenCle"
		onclick="defaultMarkerSet('cencle');"
		style="display: none; height: 30px; width: 200px; position: absolute; left: 105px; top: 10px; z-index: 9; background-color: #ffffff; text-align: center; cursor: pointer;">
		<div style="margin-top: 3px; display: inline-block;">Reset
			default location settings</div>
	</div>
	<div class="morkerModeCls2"
		style="display: none; height: 30px; width: 350px; position: absolute; left: 320px; top: 10px; z-index: 9; text-align: center; cursor: pointer;">
		<input type="text" id="searchDefaultPlace"
			style="margin-top: 3px; display: inline-block; width: 100%;"
			placeholder="Enter location">
	</div>

	<div id="map_canvas"
		style="width: 100%; height: 100%; min-width: 1080px;"></div>
</body>
</html>
