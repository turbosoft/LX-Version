...
// 						console.log("Map says: Pointer entered " + e.feature.id + " on " + e.feature.layer.name);
						...
						var kindStr = e.feature.dataKind;
						
						if (kindStr == "GeoVideo") {
							// 영상일 경우
							tmpThumbFileName1 = e.feature.thumbnail;
						}
...
// 						console.log("Map says: Pointer left " + e.feature.id + " on " + e.feature.layer.name);
					...
					featureclick: function(e) {
						...
						var featureLayer = e.feature.layer;
						
						var video = document.getElementById("video_player0");
						$("#video_player0").attr("src", ftpBaseUrl()+ '/GeoVideo/' + e.feature.filename + "#t=" + e.feature.startTime);
...
function mapCenterChange(tempArr, gpsArrLat, gpsArrLon, objXml){		//tempObj: lat, lon, file, idx, dataKind, origin, thumbnail, id
	...
	var file = tempArr[2];
	var idx = tempArr[3];
	var dataKind = tempArr[4];
	var thumbnail = tempArr[5];
	var xml = tempArr[9];
	var projectId = tempArr[11];
	var innerHTMLStrLog = "";
	...
	if (parentMarker != null) {
		...
		innerHTMLStrLog += '<li style="font-weight: 600; margin: 0px 0px 0px 72px;"><div style="padding:5px;">';
		innerHTMLStrLog += '<input type="checkbox" id="checkGps_'+i+'" style="float:left;" class="cb1" onclick="lookGps(\''+gpsArrLat+'\',\''+gpsArrLon+'\',\''+i+'\',\''+idx+'\');"/>';
		...
		innerHTMLStrFile += '<li style="font-weight: 600; margin: 0px 0px 0px 72px;"><div style="padding:5px;">';
		...
		innerHTMLStrAnno += '<li style="font-weight: 600; margin: 0px 0px 0px 72px;"><div style="padding:5px;">';
...
// 		if (dataKind == "GeoVideo") {
			
// 		} else if (dataKind == "GeoPhoto") {
// 			map.setCenter(center, 19);
// 		}
		
		// 마커 제거
// 		parentMarker.removeMarker(parentMarker.markers[0]);
// 		map.removeLayer(parentMarker);
		...
		var circleHoverStyle = new OpenLayers.Style({
			'strokeColor':'#ff0000',
			'strokeOpacity':0.8,
			'strokeWeight':1,
			'fillColor':'#ff0000',
...
// 		var lineStyleMap = new OpenLayers.StyleMap(lineStyle);
		...
		if (dataKind == "GeoVideo") {
// 			var lines = new OpenLayers.Layer.Vector("gps_line_" + idx, {styleMap: lineStyleMap});
// 			lines.parentId = "marker_" + idx;
// 			map.addLayer(lines);
			
			var circles = new OpenLayers.Layer.Vector("gps_circle_" + projectId, {
...
// 			map.addLayer(markers);
			...
			// 영상일 경우에만 해당됨.
			var circleFeatureData = new Object();
			circleFeatureData.filename = tempArr[2];
			circleFeatureData.thumbnail = tempArr[6];
			circleFeatureData.dataKind = dataKind;
			...
			for(var j=0; j<gpsArrLatgo.length; j++)
			{
				...
				circleFeatureData.startTime = j;
				addGpsMarkerCircle(gpsLon, gpsLat, circles, circleFeatureData);
				
// 				if (j != gpsArrLatgo.length - 1) {
					// 마지막은 제외한다.
// 					var gpsLat2 = gpsArrLatgo[j+1];
// 					var gpsLon2 = gpsArrLongo[j+1];
					
// 					addGpsMarkerLine(gpsLon, gpsLat, gpsLon2, gpsLat2, lines);
// 				} else if (j == gpsArrLatgo.length - 1) {
// 					var gpsLat2 = gpsArrLatgo[0];
// 					var gpsLon2 = gpsArrLongo[0];
					
// 					addGpsMarkerLine(gpsLon, gpsLat, gpsLon, gpsLat, lines);
// 				}
			}
			
// 			addGpsMarker(gpsArrLongo[0], gpsArrLatgo[0], markers, idx, 0);
		} else if (dataKind == "GeoPhoto") {
			// 이미지는 나중에
			/* var circles = new OpenLayers.Layer.Vector("gps_circle_" + projectId, {styleMap: circleStyleMap});
			circles.parentId = "marker_" + idx;
			map.addLayer(circles);
			
			var marker = null;
			var name = "marker_" + idx;
			...
			marker.markers[0].setUrl("http://maps.google.com/mapfiles/ms/icons/green-dot.png");
			addGpsMarkerCirclePhoto(marker.markers[0].lonlat, circles); */
		}
		
		else if(dataKind == "GeoTraj")
		{
			var circles = new OpenLayers.Layer.Vector("gps_circle_" + projectId, {
				styleMap: circleStyleMap
			});
...
// 				}
	...
	{
		// 다시 마커 생성
// 		fnMakeMarker(tempArr, 'load');
		
// 		if (dataKind == "GeoPhoto") {
// 			var marker = null;
// 			var name = "marker_" + idx;
...
// 			marker.markers[0].setUrl("http://maps.google.com/mapfiles/ms/icons/red-dot.png");
// 		}
		...
		// 체크해제 시
		var parentId = "marker_" + tempArr[3];
		
		var layers = map.layers;
		var delLayers = new Array();
		if (layers.length > 1) {
			for (var j = 0; j < layers.length; j++) {
				var pId = layers[j].parentId;
				
				if (pId == parentId) {
					delLayers.push(layers[j]);
				}
			}
		}
...
function addGpsMarkerCircle(gpsLon, gpsLat, circles, circleFeatureData) {
	var lonlat = new OpenLayers.LonLat(gpsLon, gpsLat);
	lonlat.transform(new OpenLayers.Projection("EPSG:4326"), map.getProjectionObject());
	var point = new OpenLayers.Geometry.Point(lonlat.lon, lonlat.lat);
	var circle = OpenLayers.Geometry.Polygon.createRegularPolygon(point, 10, 40, 0);
	
	var featurecircle = new OpenLayers.Feature.Vector(circle);
	
	// feature mouseover, mouseout, click 시 필요한 정보
	featurecircle.filename = circleFeatureData.filename;
	featurecircle.thumbnail = circleFeatureData.thumbnail;
	featurecircle.dataKind = circleFeatureData.dataKind;
	featurecircle.startTime = circleFeatureData.startTime;
...
<!-- 	 <div id="vmap" style="width:100%; height:100%;"></div> -->
	 <div id="map" style="width:100%; height:100%; cursor:default;" oncontextmenu="return false;"></div>
    <p id="bbox_result"> </p>
