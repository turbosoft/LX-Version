...
function ContentsMakes2(projectId) {
	if(loginId == null || loginId =='' || loginId == 'null') {
		...
		var tempUrl = '<c:url value="/geoCMS/upload_content.do"/>?projectId=' + projectId;
		var tempTitle = 'Row Insert';
		var tempWidth = 470;
		var tempHeight = 660;
		...
		$.ajax({
			...
			, success: function(data) {
				...
				if(data.Code == '100'){
					...
					jAlert('You can upload content after creating a layer.', 'Info');
				}
			}
		});
	}
...
// 		jAlert("로그인 정보가 만료되었습니다.\n\n다시 로그인을 수행하여 주세요.", '정보');
		...
		if((projectImage == 1 && type == "") || type == "photo"){	//image upload
