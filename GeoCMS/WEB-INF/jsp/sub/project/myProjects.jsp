...
function addProjectGroupCell(response){
	...
	innerHTML += '<div id="serverLayers" class="layerName"> <span id="serverSubject" class="serverName">Server Layers</span> <div style="float:right; margin-right:10px;"><button id="proAddBtn" onclick="openAddProjectName();">ADD</button></div> </div>';
	if(response != null){
	...
	// 				proShare = '선택공개';
				...
				outerHTML += '<div style="padding:10px;">';
	...
	// 			}
				...
				innerHTML += '<div id="pChild_'+ response[i].idx + '" style="border:1px solid gray; width:100%;"></div>';
				innerHTML += '</div>';
	...
	innerHTML += '<div id="localLayers" class="layerName"> '
	...
	$('.jstree-open').contextMenu('context3', {
		bindings: {
			...
			'context_upload': function(t) {
				...
				{
					jAlert('Please check Layer.', 'Info');
				}
			},
...
// 				gpsArr.push(response[i].gpsdata.gpsData);
				$.each(response[i].gpsdata.gpsData, function(idx, val) {
					gpsArrLat.push(val.lat);
					gpsArrLon.push(val.lon);
				});
				...
				innerHTMLStr += "<li id='subDetailNode_"+i+"' style='border-bottom: 0.5px solid #F2F3F5; border: 1px solid black; background: white; font-weight: 600; margin: 0px 0px 0px 36px;'>";
				innerHTMLStr += "<div class='imageTag' style='padding: 7px; font-size: 15px;' id='Pro_"+ response[i].datakind +"_"+response[i].idx +"' href='javascript:;'";
...
// 				innerHTMLStr += '<input type="checkbox" id="checkSub_'+i+'" style="float:left;" class="cb1" onclick="mapCenterChange(\''+tempArr+'\', \''+selectId+'\', \''+i+'\', '+gpsArr+');"/>';
				...
				innerHTMLStr += "</div>";
				innerHTMLStr += "</li>";
				innerHTMLStr += "<div id='subDetailView_"+i+"' style='font-size: 15px;'></div>"
...
function modifyProjectName(){
	...
	if(projectNameTxt == "Local Name")
	{
		jAlert('Please Modify Layer name.', 'Info');
		return;
	}
		...
		, success: function(data) {
			if(data.Code == 100){
				...
				if(response != null && response != ''){
					...
					var form = document.createElement('form');
					form.setAttribute('method','post');
					form.setAttribute('action',base_url + "/GeoVideo/geoVideo/video_write_page.do?loginToken="+loginToken+"&loginId="+loginId+'&projectBoard=1&editUserYN='+tmpEditUserYN+'&projectUserId='+response.projectuserid+'&flag='+k);
...
<!-- 		<button onclick='moveProContent();' style="float:right; margin-right:10px; color:white; border-radius: 5px;" class='offMoveCon' id='moveContentBtn'>Move Content</button> -->
	...
	<div id="project_list_table" style="height: 100%; overflow-y:scroll; overflow-x:hidden;"></div>
