...
function autoLoginSetting(id, token, type) {
	$.ajax({
		type: 'POST',
...
function menuSetting(){
	...
	menuMap.put("MyProjects",{"src": "<c:url value='/images/geoImg/main_images/execute_query.png'/>", "top": 13, "width": 70, "etc": "onclick='viewMyProjects(null);'"});
	...
	menuMap.put("searchBox",{"src": "<c:url value='/images/geoImg/btn_image/btn_search.png'/>", "top": 32, "width": 28, "etc": "alt='search Button' onclick='searchAction();'"});
	...
	if(loginType =='ADMIN') {
		innerHTMLTopMenu +=  "<img src='<c:url value='/images/geoImg/btn_image/exit_off.png'/>' style='top:15px; right:12px; position:relative; float:right; cursor:pointer; width:20px; height:20px;' id='editExitlBtn' class='editing'/>";
		innerHTMLTopMenu +=  "<img src='<c:url value='/images/geoImg/btn_image/save_off.png'/>' style='top:15px; right:42px; position:relative; float:right; cursor:pointer; width:20px; height:20px;' id='editSaveBtn' class='editing' onclick='editBtnEvent(\"SAVE\");'/>";
		innerHTMLTopMenu +=  "<img src='<c:url value='/images/geoImg/btn_image/cancel_off.png'/>' style='top:15px; right:72px; position:relative; float:right; cursor:pointer; width:20px; height:20px;' id='editCancelBtn' class='editing' onclick='editBtnEvent(\"CANCEL\");'/>";
		innerHTMLTopMenu +=  "<img src='<c:url value='/images/geoImg/btn_image/user_off.png'/>' style='top:15px; right:102px; position:relative; float:right; cursor:pointer; width:20px; height:20px;' id='manageBtn' class='editing' onclick='userManage();'/>";
		innerHTMLTopMenu +=  "<img src='<c:url value='/images/geoImg/btn_image/setting_on.png'/>' style='top:15px; right:132px; position:relative; float:right; cursor:pointer; width:20px; height:20px;' id='editBtn'/>";
	}
	...
	for(var i=0;i<menuArr.length;i++){
		...
		}else if(menuId == "searchBox"){
			//검색 박스인 경우 input box 추가
			/* innerHTMLStr += "<div style='position: relative; height: 60px; width: 430px; background: #293B5A; z-index:50; top:10px; left:10px;'>";
			//innerHTMLStr += "<div onclick='viewSearchOption(\"on\");' style=' top:14px;  float: left;  position: relative;  cursor:pointer;'><img style='width:51px;' src='<c:url value='/images/geoImg/main_images/div_menu_img.png'/>'/></div>";
			if($("#myProject_list").css("display") == "none")
			{
				innerHTMLStr += "<div onclick='viewMyProjects(null);' style=' top:14px;  float: left;  position: relative;  cursor:pointer;'><img style='width:51px;' src='<c:url value='/images/geoImg/main_images/div_menu_img.png'/>'/></div>";
			}
			else
			{
				innerHTMLStr += "<div onclick='nonViewMyProject(null);' style=' top:14px;  float: left;  position: relative;  cursor:pointer;'><img style='width:34px; margin-left:9px; margin-right:8px;' src='<c:url value='/images/geoImg/main_images/btn_before.png'/>'/></div>";	
			}
			innerHTMLStr += "<img src='" + menuMap.get(menuId).src + "' id='" + menuId + "' class='menu_images' style='width:42px; top:10px; left:327px; position:relative; cursor: pointer; clear:both;' ";
			innerHTMLStr +=  menuMap.get(menuId).etc + " /> ";
			innerHTMLStr += "<input type='text' id='srchBox' size='39' style='position:relative; left:-38px; height: 38px; top:-6px; font-family: noto_r;' onKeyPress='submit1(event);'/>";
			innerHTMLStr += "</div>" */
		}
...
// 			tmpLeft += 70;
			...
			if(loginType =='ADMIN')
			{
				innerHTMLTopMenu += "<img src='" + menuMap.get(menuId).src + "' id='" + menuId + "' class='menu_images' style='width:" + menuMap.get(menuId).width + "px; margin-top:" + (menuMap.get(menuId).top+33) + "px; left:"+ tmpLeft + "px; position:absolute; display:none; cursor: pointer; ";	
			}
			else
			{
				innerHTMLTopMenu += "<img src='" + menuMap.get(menuId).src + "' id='" + menuId + "' class='menu_images' style='width:" + menuMap.get(menuId).width + "px; margin-top:" + (menuMap.get(menuId).top+19) + "px; left:"+ tmpLeft + "px; position:absolute; display:none; cursor: pointer; ";
			}
...
// 				proShare = '선택공개';
			...
			var projectNameTxt = "";
			if (response[i].projectname != undefined && response[i].projectname.length > 0) {
				projectNameTxt = response[i].projectname.length>40? response[i].projectname.substring(0,40)+'...' : response[i].projectname;
			}
			...
			var tmpUserId = "";
			if (response[i].projectid != undefined && response[i].projectid.length > 0) {
				tmpUserId = response[i].projectid.length>7? response[i].projectid.substring(0,7)+'...' : response[i].projectid;
			}
...
function nonViewMyProject(orderIdx)
{
