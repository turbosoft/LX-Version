...
//		File removeFile = null;
		...
		if(isServerUrl){
			try{
				...
				ftp.connect(b_serverUrl, Integer.parseInt(b_serverPort)); // �������� " "�ȿ� ���� �ּ� �Է� �Ǵ� "�����ּ�", ��Ʈ��ȣ 
			    ...
			    ftp.changeWorkingDirectory(b_serverPath +"/GeoVideo"); // �۾� ���丮 ����
			    reply = ftp.getReplyCode();
			    if (reply == 550) {
						...
						if("2".equals(gpsFileTypeData)){
							...
							for(int k=0;k<filesXml.size();k++){
								...
								if(k == 0){
									...
									String timeToElement = eElement2.getFirstChild().toString();
									String a = eElement2.getFirstChild().getNodeValue();
								...
								if(idxArrStr != null && !"".equals(idxArrStr)){
									...
									if(idxArr != null && idxArr.length > 0){
										for(int m=0; m<idxArr.length; m++){
											...
											param4.put("xmlData", "");
											dataDao.updateVideo(param4);
										    System.out.println(tmpGpxFilePathDir + File.separator + nowFileName + "���� FTP ���ε� ����");
										}
										////////////////////////////////////////////
